...
bool ConfigureAction::fromJson(const Json::Value& action_json) {
  if (!WebrtcRosJsonParser::GetStringFromJsonObject(action_json, kTypeFieldName, &type)) return false;
  properties.clear();
...
void ConfigureAction::toJson(Json::Value* action_json) const { (*action_json)[kTypeFieldName] = type; }
...
bool ConfigureMessage::fromJson(const Json::Value& message_json) {
  if (isConfigure(message_json)) {
    ...
    if (!action_array_json.isArray()) return false;
