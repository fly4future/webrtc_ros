...
bool IceCandidateMessage::fromJson(const Json::Value& message_json) {
  if (isIceCandidate(message_json)) {
    if (!WebrtcRosJsonParser::GetStringFromJsonObject(message_json, kSdpMidFieldName, &sdp_mid)) return false;
    if (!WebrtcRosJsonParser::GetIntFromJsonObject(message_json, kSdpMlineIndexFieldName, &sdp_mline_index))
      return false;
    if (!WebrtcRosJsonParser::GetStringFromJsonObject(message_json, kCandidateFieldName, &candidate)) return false;
    return true;
  } else
    return false;
}
