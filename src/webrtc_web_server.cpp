...
public:
  WebrtcWebServerImpl(rclcpp::Node::SharedPtr nh, int port, SignalingChannelCallback callback, void *data)
    ...
    handler_group_.addHandlerForPath("/list_streams.json", std::bind(&WebrtcWebServerImpl::handle_list_streams, this, std::placeholders::_1,
    ...
    handler_group_.addHandlerForPath("/webrtc", async_web_server_cpp::WebsocketHttpRequestHandler(std::bind(&WebrtcWebServerImpl::handle_webrtc_websocket, this,
                                                                                                            std::placeholders::_1, std::placeholders::_2)));
    handler_group_.addHandlerForPath(
        "/.+", async_web_server_cpp::HttpReply::from_filesystem(async_web_server_cpp::HttpReply::ok, "/", webrtc_ros_path + "/web", false, any_origin_headers));
...
private:
  ...
  static bool ros_connection_logger(async_web_server_cpp::HttpServerRequestHandler forward, const async_web_server_cpp::HttpRequest &request,
                                    async_web_server_cpp::HttpConnectionPtr connection, const char *begin, const char *end) {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("webrtc"), connection->socket().remote_endpoint().address().to_string()
