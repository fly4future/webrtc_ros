...
class SignalingChannelImpl : public SignalingChannel {
 public:
  ...
  void sendTextMessage(const std::string& message) { websocket_->sendTextMessage(message); }
...
class WebrtcWebServerImpl : public WebrtcWebServer {
 public:
  WebrtcWebServerImpl(int port, SignalingChannelCallback callback, void* data)
    ...
    handler_group_.addHandlerForPath(
        "/", async_web_server_cpp::HttpReply::from_file(async_web_server_cpp::HttpReply::ok, "text/html",
                                                        ros::package::getPath("webrtc_ros") + "/web/index.html",
                                                        any_origin_headers));
    handler_group_.addHandlerForPath("/list_streams.json",
                                     boost::bind(&WebrtcWebServerImpl::handle_list_streams, this, _1, _2, _3, _4));
    handler_group_.addHandlerForPath(
        "/viewer", async_web_server_cpp::HttpReply::from_file(async_web_server_cpp::HttpReply::ok, "text/html",
                                                              ros::package::getPath("webrtc_ros") + "/web/viewer.html",
                                                              any_origin_headers));
    handler_group_.addHandlerForPath("/webrtc", async_web_server_cpp::WebsocketHttpRequestHandler(boost::bind(
                                                    &WebrtcWebServerImpl::handle_webrtc_websocket, this, _1, _2)));
    handler_group_.addHandlerForPath(
        "/.+", async_web_server_cpp::HttpReply::from_filesystem(async_web_server_cpp::HttpReply::ok, "/",
                                                                ros::package::getPath("webrtc_ros") + "/web", false,
                                                                any_origin_headers));
    server_.reset(
 ...
 private:
  ...
  async_web_server_cpp::WebsocketConnection::MessageHandler handle_webrtc_websocket(
      const async_web_server_cpp::HttpRequest& request, async_web_server_cpp::WebsocketConnectionPtr websocket) {
