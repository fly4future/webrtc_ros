...
ImageTransportFactory::Subscriber ImageTransportFactory::subscribe(const std::string& topic, const Callback& cb,
...
ImageTransportFactory::Dispatcher::Dispatcher(image_transport::ImageTransport& it, const std::string& topic,
                                              const std::string& transport)
    : sub_(it.subscribe(topic, 1, boost::bind(&Dispatcher::dispatch, this, _1), ros::VoidPtr(),
                        image_transport::TransportHints(transport))),
      next_id_(1) {
...
ImageTransportFactory::Dispatcher::~Dispatcher() {
  ...
  if (callbacks_.size() > 0) ROS_ERROR("BUG in ImageTransportFactory: %zu orphaned subscriber(s)", callbacks_.size());
}
...
void ImageTransportFactory::Subscriber::shutdown() {
  if (data_) {
    if (data_->d_) data_->d_->removeCallback(data_->id_);
    data_->d_.reset();
    data_.reset();
  }
}
