...
WebrtcClientObserverProxy::WebrtcClientObserverProxy(WebrtcClientWeakPtr client_weak) : client_weak_(client_weak) {}
...
void WebrtcClientObserverProxy::OnIceCandidate(const webrtc::IceCandidateInterface* candidate) {
  WebrtcClientPtr client = client_weak_.lock();
  if (client) client->OnIceCandidate(candidate);
}
...
WebrtcClient::WebrtcClient(ros::NodeHandle& nh, const ImageTransportFactory& itf, const std::string& transport,
  ...
  peer_connection_factory_ = webrtc::CreatePeerConnectionFactory(
      ...
      std::unique_ptr<webrtc::VideoDecoderFactory>(
          new webrtc::MultiplexDecoderFactory(std::make_unique<webrtc::InternalDecoderFactory>())),
      nullptr, nullptr);
...
void WebrtcClient::init(boost::shared_ptr<WebrtcClient>& keep_alive_ptr) { keep_alive_this_ = keep_alive_ptr; }
...
bool WebrtcClient::initPeerConnection() {
  ...
  if (!peer_connection_) {
    ...
    peer_connection_ =
        peer_connection_factory_->CreatePeerConnection(config, nullptr, nullptr, webrtc_observer_proxy_.get());
...
class MessageHandlerImpl : public MessageHandler {
 public:
  ...
  void handle_message(MessageHandler::Type type, const std::string& raw) {
    ...
    if (_this)
      _this->signaling_thread_->Invoke<void>(RTC_FROM_HERE,
...
MessageHandler* WebrtcClient::createMessageHandler() { return new MessageHandlerImpl(keep_alive_this_); }
...
#define FIND_PROPERTY_OR_CONTINUE(key, name)                    \
  ...
  std::string name = action.properties.at(key)
        ...
        if (action.type == ConfigureAction::kAddStreamActionName) {
          ...
          if (video_type == "ros_image") {
            ...
            rtc::scoped_refptr<RosVideoCapturer> capturer(
                new rtc::RefCountedObject<RosVideoCapturer>(itf_, video_path, transport_));
          ...
          if (audio_type == "local") {
            ...
            rtc::scoped_refptr<webrtc::AudioTrackInterface> audio_track(peer_connection_factory_->CreateAudioTrack(
                track_id, peer_connection_factory_->CreateAudioSource(options)));
      ...
      rtc::scoped_refptr<DummySetSessionDescriptionObserver> dummy_set_description_observer(
